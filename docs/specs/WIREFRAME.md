...
- **Export section:**
  ...
  - Export Format: single-select dropdown (KML, KMZ, JSON, MAVLink, UgCS, WPML, GPX, Litchi, CSV, DroneDeploy) defaulting to KMZ, styled like the DJI heading-mode picker and carrying a "Format" field label. Only the currently-selected format's description and capability note render beneath it. One format per export - the download sends a one-element `formats` array. Strings under `mission.validationExportPage.formatLabel` (EN + SK).
  ...
  - **Export grayed out / disabled** until the plan reaches VALIDATED; it stays enabled through EXPORTED and MEASURED (re-download a measured mission's plan) and is disabled again in the terminal COMPLETED / CANCELLED states. Shows info note: "Validate the plan first" while still pre-validation
...
**Body** (`ResultsPage`, since #76 — Download PDF moved into the `MeasurementTabNav` header; since #95 — the run-name / status / Rename / Delete sub-header was removed, those controls now live on the header picker pill; since #163 — the flat card stack became an anchored sectioned layout; since #166 — the body is a 30/70 split mirroring `MissionTabNav`): the workspace below the header splits into a left 30% column and a right 70% column. `MeasurementTabNav` owns both columns and hands the left column down as a portal target on its outlet context (`leftPanelEl`), plus the current run's list row (`currentRow`); `ResultsPage` portals a stacked `ResultsLeftPanel` into that slot and renders the result sections into the right column. Everything below renders only once `has_results` is true; otherwise a single "not ready yet" card.

**Left panel** (`ResultsLeftPanel`, since #166) — a stack of `--tv-*` cards: a **measurement summary** (run display name + method, sequence order, runway heading, the `MeasurementStatusChip`, and the processed date off `currentRow.created_at`); an **overall verdict** chip (`PASS` unless a scored light failed, `FAIL` on any fail, `Pending` until something is scored); a **per-PAPI verdict list** (PAPI_A–D, each row showing measured vs `setting_angle ± tolerance` and a pass/fail/neutral icon, color-dotted by inspection-light color); the **glide-path angle** (midpoint of PAPI_B's upper transition and PAPI_C's lower transition; "Not available" when either edge is missing); and a **Jump to section** nav that smooth-scrolls to the right-column sections. `overallVerdict` and `computeGlidePathAngle` are pure helpers exported for test.

**Right column** — five anchored `<section>`s, in order, each with an `id` the left-panel nav scrolls to:
1. **PAPI Vertical Analysis** (`papi-vertical`) — three stacked blocks:
...
2. **PAPI Horizontal Analysis** (`papi-horizontal`, since #167) — `HorizontalAnalysisSection` plots red chromaticity and luminosity vs horizontal angle (centerline at 0°), plus a derived `LightDirectionCard` table giving each unit's horizontal angle of maximum luminosity.
3. **Drone Path** (`drone-path`) — `DronePathMap` (MapLibre drone-path map with reference points) beside `DroneHeightProfileChart` (since #168, replacing the old `ClimbProfileChart`): a dual-axis recharts line chart plotting the flown `drone_path` WGS84 elevation on the left axis, plus one dashed altitude-difference series (right axis) per PAPI reference point that carries an `elevation`, plus a touch-point series if a `TOUCH_POINT` reference with elevation ever appears (forward-compatible, omitted otherwise). Its empty state shows only when no path point records an elevation. PAPI footage holds a fixed standoff while altitude sweeps, so the flown path often collapses to a near-stationary point: the map draws circle markers over the path points (not just the line) so a stationary/single-point path stays visible, and falls back to a fixed zoom when the combined path+reference bbox is degenerate so `fitBounds` can't over-zoom (#73). The "no path" empty state shows only when `drone_path` is genuinely empty.
4. **Annotated Video** (`annotated-video`) — `AnnotatedVideoPlayer` (defaults to "All PAPI lights"; a fixed `aspect-video` box the video stretches to fill via `object-fill` so tall per-light crops share the same frame as the full track, plus a Fullscreen button). The PDF download lives in the header, not the body.
5. **Data Tables** (`data-tables`) — the `TransitionAngleTable` (each PAPI light's measured transition angle vs `setting_angle ± tolerance`, with a solid PASS/FAIL verdict tone); since #166 it moved out of the per-light grid into its own section. Since #168 a **Data tables** sub-section follows in a 2-col grid with four tables derived client-side from the same `data` payload (no backend or type changes): `TransitionDifferenceTable` (per-light start / middle (80%) / end / width / nominal / correction + PASS/FAIL/UNKNOWN verdict, with a pairwise consecutive-light diff sub-block), `GlidePathSummaryTable` (glide-path-to-PAPI angle from `PAPI_B.transition_angle_max` + `PAPI_C.transition_angle_min` with nominal/tolerance/status — the glide-path-to-touch-point row renders `—`, gated on a touch-point datum the current contract never provides), `ChromaticityComparisonTable` (per-light red-chromaticity min/max/avg/range as percent, from `chromaticity_x`), and `LuminosityComparisonTable` (per-light intensity min/max/avg/range). The two comparison tables share `StatsComparisonTable` plus the `seriesStats()` helper (`resultsStats.ts`).

**States:** loading spinner, error card, and the `has_results: false` "not ready" card. Strings under `results.*` (incl. `results.sections.*`, `results.summary.*`, `results.verdictRollup.*`, `results.perPapi.*`, `results.glidePath.*`, `results.perLight.*`, `results.vertical.*`, `results.horizontal.*`, `results.direction.*`, `results.charts.*`, `results.heightProfile.*`, `results.dataTables.*`, `results.transitionDiff.*`, `results.chromaticityCompare.*`, `results.luminosityCompare.*`; the orphaned `results.climb.*` block was removed with `ClimbProfileChart` in #168) + `measurement.*` (EN + SK).
