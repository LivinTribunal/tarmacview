...
class OptimizedOverlayRenderer:
    ...
    @staticmethod
    def add_drone_position_overlay_optimized(frame: np.ndarray, frame_number: int,
            ...
            for papi_name in ['PAPI_A', 'PAPI_B', 'PAPI_C', 'PAPI_D', 'PAPI_E', 'PAPI_F', 'PAPI_G', 'PAPI_H']:
                if papi_name in reference_points:
                    try:
                        ...
                        drone_elevation_wgs84 = drone_data.get('elevation_wgs84', drone_data.get('elevation', 0))
                        papi_elevation_wgs84 = papi_point.get('elevation_wgs84', papi_point.get('elevation', 0))
        ...
        if reference_points and 'TOUCH_POINT' in reference_points:
            try:
                ...
                # Use elevation_wgs84 with fallback to elevation for backward compatibility
                drone_elevation_wgs84 = drone_data.get('elevation_wgs84', drone_data.get('elevation', 0))
                touch_elevation_wgs84 = touch_point.get('elevation_wgs84', touch_point.get('elevation', 0))
        ...
        cv2.putText(frame, "GPS POSITION", (x_col1, y_base),
                   font, 1.0, (80, 80, 80), 2)
        cv2.putText(frame, f"Lat: {drone_data.get('latitude', 0):.6f}", (x_col1, y_base + 45),
                   font, 0.8, (60, 60, 60), 2)
        cv2.putText(frame, f"Lon: {drone_data.get('longitude', 0):.6f}", (x_col1, y_base + 85),
                   font, 0.8, (60, 60, 60), 2)
        cv2.putText(frame, f"Alt: {drone_data.get('elevation_wgs84', drone_data.get('elevation', 0)):.1f}m WGS84", (x_col1, y_base + 125),
                   font, 0.8, (60, 60, 60), 2)
        ...
        for papi_name, angle in papi_angles.items():
            ...
            transition_mid = papi_ref.get('transition_angle_middle', 'N/A')
            ...
            if isinstance(transition_start, (int, float)) and isinstance(transition_end, (int, float)):
        ...
        if touch_angle is not None:
            cv2.putText(frame, "TOUCH POINT", (x_col3, y_base),
                       font, 1.0, (80, 80, 80), 2)
            cv2.putText(frame, f"Current Angle: {touch_angle:.2f}", (x_col3, y_base + 45),
            ...
            glide_path_angle = touch_ref.get('nominal_angle', touch_ref.get('glide_path_angle', 3.0))
    ...
    @staticmethod
    def add_overlays_to_frame_with_tracking(frame: np.ndarray, tracked_positions: Dict, 
        ...
        for light_name, pos in tracked_positions.items():
            ...
            if measurements_data and frame_number < len(measurements_data):
                ...
                measured_width = int(measurement_data.get(f'{light_key}_width', pos.get('size', 20)))
                measured_height = int(measurement_data.get(f'{light_key}_height', pos.get('size', 20)))
            ...
            # Draw text
            cv2.putText(enhanced_frame, label, (int(text_x + 2), int(text_y)),
        ...
        enhanced_frame = InfoOverlayRenderer.add_angle_overlay(
            enhanced_frame, frame_number, drone_telemetry, reference_points,
            real_gps_data, fps, frame_measurements
        )
