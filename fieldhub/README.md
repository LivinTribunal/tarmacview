...
For field day, the repo-root launchers do everything below in one command —
detect the LAN IP (or take it as `$1`), generate certs, fill `.env.docker`
non-destructively, and bring the stack up. Safe to re-run:

```bash
./start-field.sh            # macOS / Linux (auto-detect IP)
./start-field.sh 192.168.8.100   # or pin the laptop's LAN IP
start-field.bat             # Windows (double-click, or pass the IP)
```

They print the hub URL for DJI Pilot 2 (`https://<ip>:8443`) and the reminder
to install `certs/ca/ca.crt` on each RC once. The manual sequence below is the
same steps spelled out.
