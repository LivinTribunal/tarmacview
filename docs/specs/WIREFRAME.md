...
**Header action:**
- **"Upload Drone Media" button** — secondary button with an upload icon in the MissionTabNav action slot (previously disabled). Opens the **Upload Drone Media dialog**: drone-media files reported by the field hub, grouped by matched mission with a file-count badge per group, plus an *Unassigned* group for files matching could not place. Each file row shows the filename (object-key tail), the capture time as `dd/mm/yyyy HH:MM` ("No capture time" when the device reported none), and a dropdown that reassigns the file to any of the airport's missions or back to *Unassigned*. Each mission group carries a **Confirm Ingest** button that marks the group's files `INGESTED` (idempotent; ingested files disappear from the dialog). Opening the dialog — and every reassign/confirm — refetches `GET /api/v1/drone-media`, which also retries matching for lingering `RECEIVED` files. Empty state: "No drone media received yet. Upload media from DJI Pilot 2 after landing." Load/assign/confirm failures surface as an inline error line. EN + SK strings under `mission.uploadDroneMediaDialog.*`; button label under `mission.validationExportPage.uploadDroneMedia`. See SPEC.md "Drone media matching + upload dialog" and `docs/specs/FIELD-HUB.md` §5.
