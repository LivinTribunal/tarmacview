...
- **Complete / Cancel / Delete:**
  - Complete: sets COMPLETED (terminal state). Available once EXPORTED or MEASURED.
  - Cancel: sets CANCELLED (terminal state). Available once EXPORTED or MEASURED.
  - Delete: removes from database entirely. Available at any status. Confirmation dialog required.
  - Complete and Cancel grayed out / disabled until EXPORTED or MEASURED.
...
**Body** (`ResultsPage`, since #76 — Download PDF moved into the `MeasurementTabNav` header): a `--tv-*` card stack led by a sub-header showing the run name (operator `label` when set, else the `Inspection N · Method` fallback), the run status, and two buttons — **Rename** (modal → `PATCH /api/v1/measurements/{id}`; blank clears to the fallback) and **Delete** (confirm modal → `DELETE /api/v1/measurements/{id}`, then navigates back to the list, Page 15). The card stack renders only once `has_results` is true; otherwise a "not ready yet" card. Four content blocks:
