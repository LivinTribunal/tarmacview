...
**agl** — surface_id (FK), agl_type (VARCHAR 30), name, position (PointZ 4326), side (VARCHAR 10), glide_slope_angle, glide_slope_angle_tolerance (Float, nullable, default `DEFAULT_GLIDE_SLOPE_ANGLE_TOLERANCE_DEG = 0.1`, gt 0 — results-time verdict band in degrees for the measured glidepath vs `glide_slope_angle`; coordinator-edited alongside `glide_slope_angle`, PAPI-only, nulled on a PAPI → edge-lights switch; NOT a trajectory input; migration `0022_glide_slope_tolerance`), distance_from_threshold, meht_height_m (Float, nullable — surveyed/published minimum eye height over the threshold in meters; PAPI-only, coordinator-edited, nulled on a PAPI → edge-lights switch. When set, MEHT_CHECK and APPROACH_DESCENT fly this height; when null it is derived from `distance_from_threshold · tan(glide_slope)` at trajectory time via `resolve_meht_height`, so a null column is byte-identical to the pre-feature output. Not enforced PAPI-only at the schema layer (the form gates it); additive nullable migration `0024_agl_meht_height`), offset_from_centerline
...
**inspection_configuration** — altitude_offset, measurement_speed_override, measurement_density (INTEGER), custom_tolerances, density, hover_duration, horizontal_distance, sweep_angle, angle_source (VARCHAR 10, nullable, default `'CUSTOM'` — `'PAPI'` resolves the VP climb bookends from `setting_angles` ± offsets at compile; `'CUSTOM'` uses `angle_start` / `angle_end` directly), angle_start / angle_end (Float, nullable, the VP climb bookends in degrees, both clamped to `[1.0°, 16.5°]`; null falls back to legacy `1.9°` / `6.5°`), angle_offset_above / angle_offset_below (Float, nullable, PAPI-mode offsets added above `max(setting_angles)` and subtracted below `min(setting_angles)` for the VP climb top/start; `angle_offset_above` also drives the HORIZONTAL_RANGE arc-side angle), descent_start_distance (Float, nullable, APPROACH_DESCENT only — meters back of the runway threshold where the descent begins; null falls back to `DEFAULT_DESCENT_START_DISTANCE = 1000 m`), descent_glide_slope_override (Float, nullable, APPROACH_DESCENT only — operator-pinned descent angle in degrees that overrides the PAPI-derived glide slope; null uses the resolved PAPI angle), papi_center_height_reference (VARCHAR 10, nullable, server-default `'GROUND'`, CHECK rendered from the `PapiCenterHeightReference` enum — the camera center-height reference for the three glide-slope methods (HORIZONTAL_RANGE / VERTICAL_PROFILE / APPROACH_DESCENT); `GROUND` aims the camera at the LHA centroid at ground level (unchanged default), `LENS` raises the aim altitude by the average `lens_height_agl_m` across the selected PAPI LHAs (falls back to GROUND when none are set), `CUSTOM` raises it by `papi_center_height_custom_m`. The lift is applied to the LHA-centroid aim in the trajectory orchestrator (`_inspection_pass`) and again on revalidate, so existing GROUND inspections stay byte-identical; migration `0022_papi_center_height_reference` adds both columns, `0023_merge_0022_heads` unifies the two 0022 heads), papi_center_height_custom_m (Float, nullable, ge 0 — CUSTOM-mode height in meters above ground), scan_surface_id (UUID FK → airfield_surface, ON DELETE SET NULL, SURFACE_SCAN only — the target surface; SURFACE_SCAN is AGL-agnostic and targets a surface, not an LHA), scan_length_mode (VARCHAR 20, nullable, `FULL | MAX_LENGTH | INTERVAL` — FULL scans the whole surface, MAX_LENGTH caps the far end at `scan_length_to`, INTERVAL trims both ends), scan_length_from / scan_length_to (Float, nullable — along-track window in meters measured from the surface origin; INTERVAL with `from >= to` is rejected at the schema boundary with 422), scan_length_anchor (VARCHAR 20, nullable, `THRESHOLD | ENDPOINT` — which runway end the `MAX_LENGTH` cap and `INTERVAL` window are measured from; null = THRESHOLD, the default, reproducing the threshold-anchored behaviour byte-for-byte. ENDPOINT reflects the window to `[total_length − to, total_length − from]`, so "scan the first N m" means "from the far endpoint". Orthogonal to the Direction toggle — Direction reorders the serpentine traversal, the anchor only moves where the window sits; CHECK + column added by migration `0021_scan_length_anchor`), scan_width (Float, nullable — perpendicular band width in meters; null = full surface width centered on the centerline), scan_width_side (VARCHAR 10, nullable, `LEFT | RIGHT` — which side of the axis a narrowed band sits on), scan_height (Float, nullable — commanded AGL of the pass; null falls back to `DEFAULT_SURFACE_SCAN_HEIGHT = 10 m`), scan_run_count (Integer, nullable, ≥ 1 — number of serpentine runs; null auto-derives from the FOV footprint and sidelap), scan_run_orientation (VARCHAR 20, nullable, `LENGTH_WISE | WIDTH_WISE` — runs laid parallel to or across the axis), scan_sidelap_percent (Float, nullable, 0–80 — overlap between adjacent runs; null falls back to `DEFAULT_SURFACE_SCAN_SIDELAP_PERCENT = 20`), scan_frontlap_percent (Float, nullable, 0–80 — forward overlap between consecutive photos along each run; null falls back to `DEFAULT_SURFACE_SCAN_FRONTLAP_PERCENT = 0`, which reproduces the legacy footprint-spaced tiling). SURFACE_SCAN also reuses the existing `camera_gimbal_angle`, `capture_mode`, `measurement_speed_override`, `direction`, and `altitude_offset` columns. (CHECK constraints on the three scan enum columns + the FK are added by migration `0015_surface_scan_config`; the additive nullable `scan_frontlap_percent` column lands in `0016_surface_scan_frontlap`.) lha_ids (JSONB — array of UUID), lha_selection_rules (JSONB, NOT NULL, default `{}` — per-AGL helper rule keyed by `agl_id`; resolved at write time into the canonical `lha_ids` list above. Empty `{}` means "no rule recorded; treat as CUSTOM".)

### Mission & Inspection

**mission** — name, status (MissionStatus enum), created_at, updated_at, operator_notes, drone_profile_id (FK), date_time, default_speed, measurement_speed_override, default_altitude_offset, takeoff_coordinate (PointZ 4326), landing_coordinate (PointZ 4326), dji_heading_mode (`'smoothTransition' | 'towardPOI' | 'followWayline'`, default `'towardPOI'` since migration `0022` — operator's last-used per-export preference; new missions persist `towardPOI` (continuous POI tracking), existing rows keep their value; only consumed by KMZ/WPML generators, NOT in TRAJECTORY_FIELDS so flipping it never regresses status), keep_inside_airport_boundary (Boolean, NOT NULL, default `true` - biases the A* transit pathfinder to stay inside the airport-boundary polygon and gates the boundary-egress safety warning; in TRAJECTORY_FIELDS so toggling regresses to DRAFT), constraints (via constraint_rule)
...
The verification half of the plan→fly→measure loop: scores an inspection's flown PAPI footage against the snapshotted `LHA` ground truth, run by the Celery video-processing engine. Design: `docs/specs/TARMACVIEW-MERGE-PLAN.md` §6/§7.2/§8. The measurement aggregate is the `Measurement` ORM model in `app/models/measurement.py` — it owns the status machine, reference-point snapshotting, and per-light scoring directly; the `app/services/measurement_service/` package orchestrates the row and the engine runners.

**measurement** — one inspection's measurement run. inspection_id (FK → inspection, ON DELETE CASCADE, indexed), status (MeasurementStatus enum, CHECK generated from the enum via `enum_check_values`), label (String, nullable — operator-supplied free-text run name; a blank/whitespace value clears it back to the inspection-label fallback `Inspection N · Method`), runway_heading (Float, nullable — parent runway heading for the horizontal-angle calc), glide_slope_angle (Float, nullable — the configured glide slope snapshotted off the inspection's AGL at create time; an audit record, NOT a live join), glide_slope_angle_tolerance (Float, nullable — the AGL's `glide_slope_angle_tolerance` snapshotted at create time, falling back to `DEFAULT_GLIDE_SLOPE_ANGLE_TOLERANCE_DEG = 0.1` when the AGL leaves it unset, but only when there is a configured angle to band around — no angle, no tolerance), reference_points (JSONB, default `[]` — the snapshotted LHA ground truth: light name, position, setting_angle, tolerance captured at create time; an audit record, NOT a live join, so a later LHA edit can't change a finished run's pass/fail), light_boxes (JSONB, default `[]` — confirmed first-frame light boxes in percentage coords; operator-confirmed on the manual `AWAITING_CONFIRM` path, detector-confirmed when a confident detection auto-confirms), summaries (JSONB, default `[]` — per-light PASS/FAIL rollup vs `setting_angle ± tolerance`), media_object_keys (JSONB, default `[]` — ordered input video object keys pulled from the inspection's media), first_frame_object_key (String, nullable — pointer to the extracted first-frame image in object storage), object_key (String, nullable — pointer to the gzipped per-frame results json; the heavy blob never lands in Postgres), annotated_video_keys (JSONB, default `{}` — annotated output videos keyed by light name + enhanced/combined), error_message (Text, nullable — set only on ERROR), created_at / updated_at (onupdate now()). Migration `0018_measurement` (**T3**); the additive nullable `label` column lands in `0019_measurement_label`; the snapshot columns `glide_slope_angle` / `glide_slope_angle_tolerance` land in `0022_glide_slope_tolerance`. The PAPI light slots are `PAPI_A`..`PAPI_D` left-to-right.

The delete/rename endpoints remain available — `DELETE /api/v1/measurements/{id}` drops the aggregate and its object-storage artifacts (the gzipped results blob, the first frame, every annotated video) best-effort after commit; `PATCH /api/v1/measurements/{id}` sets/clears `label`. No operator UI exposes them after the Results relocation into the mission workspace (operators re-measure from Validation & Export instead).
...
DRAFT → PLANNED → VALIDATED → EXPORTED → MEASURED → COMPLETED
  ...
  any non-terminal status (DRAFT/PLANNED/VALIDATED/EXPORTED/MEASURED) → CANCELLED
        (operator can abandon a mission at any point in its lifecycle;
         COMPLETED is reachable only from MEASURED)
```
...
- any non-terminal status → CANCELLED: operator abandons mission (reachable from DRAFT/PLANNED/VALIDATED/EXPORTED/MEASURED, not only MEASURED)

**MEASURED trigger:** the transition fires on *measurement kickoff* — the moment a mission's first measurement run is created (`measurement_service.create_measurement` calls `Mission.mark_measured()` before the flush). It is idempotent: a multi-inspection mission hits create more than once and only the first call (while still VALIDATED/EXPORTED) transitions; later calls no-op. MEASURED is intentionally NOT in `TERMINAL_STATUSES`, `POST_PLAN_STATUSES`, or the edit gates, so it stays deletable and never auto-regresses. The one status-set it joins is `EXPORT_ELIGIBLE_STATUSES` (the export/dispatch gate): a measured mission can still re-export / re-dispatch because its plan + artifacts persist, and re-export keeps it MEASURED (the `VALIDATED → EXPORTED` bump keys on VALIDATED only).
...
- Complete button: enabled only at MEASURED - a mission must be measured before it can be completed
- Cancel button: enabled from any non-terminal status (everything except COMPLETED/CANCELLED), MEASURED included; routes through a confirm-warning modal
- Delete button: enabled at any status (terminal states included); routes through a confirm-warning modal
- COMPLETED and CANCELLED are terminal states — no further transitions, but the mission can still be deleted
...
alt_i       = meht_point.alt + remaining_i · tan(α),   meht_point.alt = threshold.alt + meht_height
```
On-axis descent down the runway centerline, anchored on the **MEHT point over the threshold** — the threshold position lifted to `threshold.alt + meht_height`, where `meht_height` comes from `resolve_meht_height` (surveyed `agl.meht_height_m` if set, else `distance_from_threshold · tan(glide_slope)`). Starts `descent_start_distance` D (default `DEFAULT_DESCENT_START_DISTANCE = 1000 m`) back of the threshold along the approach axis, descends at angle α (`descent_glide_slope_override` if set, else the PAPI-derived glide slope), and **terminates with a HOVER + capture at the MEHT point** (PHOTO_CAPTURE, or RECORDING under video pre-wrap; the hover carries `hover_duration`, default `DEFAULT_MEHT_HOVER_DURATION`) — so one approach inspection yields both the descent series and the MEHT measurement. Camera framed on the LHA center; the runway surface must have a threshold position and the AGL a resolvable MEHT height (`meht_height_m` or `distance_from_threshold`) or compile raises `TrajectoryGenerationError`. Counted as a PAPI method for terrain handling — terrain post-processing reuses `_apply_papi_glide_slope_terrain` anchored on the MEHT point (not the LHA), so commanded elevation angle survives terrain undulation.

**MEHT_CHECK** (minimum eye height over threshold — ICAO Doc 9157 P4 §8.3.43):
A single HOVER + capture positioned **directly over the runway threshold** on the centerline at `threshold.alt + meht_height + altitude_offset`. This is the flown position, so `altitude_offset` is baked into the anchor here (MEHT_CHECK runs no terrain rebuild). `meht_height` resolves the same way as APPROACH_DESCENT — surveyed `agl.meht_height_m` if set, else `distance_from_threshold · tan(glide_slope)` via `resolve_meht_height`. Camera framed on the LHA center; heading and gimbal aim at the LHA centroid. Requires a runway threshold position and a resolvable MEHT height or compile raises `TrajectoryGenerationError`.
...
Left: per-constraint breakdown (pass/fail/warning), "Edit Configuration" → Config tab, "Accept" → VALIDATED. Right: map + export section (single-select format dropdown defaulting to KMZ, download button). Export disabled until VALIDATED. Complete enabled only at MEASURED; Cancel enabled from any non-terminal status; Delete available always. The *Upload Drone Media* button in the MissionTabNav header opens the drone-media dialog — see "Drone media matching + upload dialog" below.

### Page 08b — Mission Results (`/operator-center/missions/:id/results`)
The fifth mission tab (after Validation & Export), rendered through `MissionTabNav`. Left: an inspection picker (single-select, result tag per row; DONE rows drill down, un-measured rows disabled, and an AWAITING_CONFIRM row is reviewable — clicking it opens `MeasurementFlowDialog` for the manual first-frame box-confirm step, which calls `confirmMeasurementLights` to advance the run AWAITING_CONFIRM → PROCESSING) → Inspection Info panel (method, sequence, runway heading, status, processed date, overall verdict, glide-path angle) → Per LHA verdict (one row per light in `lights[]`). On load the tab **lands on a protocol-style mission overview** (`MissionResultsOverview`), not a single inspection: a session-header card, a placeholder weather card, one `DeviceProtocolSection` per device grouped by runway (glide slope + per-light transition rows filled where measured, greyed `N/A` rows for parameters we don't compute yet, plus placeholder ALS/RLS serviceability sections), a mission-level evaluation table, and a recommendations block. Un-measured parameters render visually distinct from a real FAIL. The single-inspection view becomes a **drill-down**: reached by clicking a DONE device in the overview, by picking a DONE inspection in the left panel, or via the `?inspection=` deep-link; a "← Back to overview" control returns. Right: in drill-down, the result sections with Data Tables first. Header action slot: a disabled Save pill + Saved-timestamp and a Download PDF Report button **enabled only in the drill-down** (mission-scale PDF is a non-goal). New read-only endpoint `GET /api/v1/missions/{mission_id}/results` (`MissionResultsResponse`, operator-gated, no audit row) backs the overview; the drill-down still reuses `getMeasurementResults` / `downloadMeasurementReport`, plus the review dialog's existing `GET /measurements/{id}` + `POST /measurements/{id}/confirm-lights`. Old `/operator-center/measurements*` deep-links redirect here. Full detail in WIREFRAME Page 15/16.
...
`GET /api/v1/field-link/status` (operator JWT) reports the field hub link as `{hub_online, rc_connected, broker_connected, devices: [{sn, model_name, model_key, domain, online, bound, gateway_sn}], connect_url, public_host}`. Since #109 the four state signals are distinct: `hub_online` (backend reaches the hub), `rc_connected` (Pilot's HTTP session is live — a hub-side heartbeat refreshed on login + every authenticated request), `broker_connected` (the hub↔broker link), and `devices[].online` (a drone actually live on MQTT). The backend proxies the hub's shared-secret internal status endpoint (`X-Hub-Secret`); an unset `FIELDHUB_URL` means "no hub in this deployment" and returns `hub_online=false` without a network attempt, and a down/unreachable/malformed hub degrades to the same shape — the route never 500s over hub state. `connect_url` / `public_host` (added in #28) carry the device-facing connect address and are `null` on the degraded / no-host shape.

`ExportPanel` owns the poll via `useFieldLinkStatus` (10 s while mounted) and reads only `status`, handing it to `ExportFormatSection` for the *Send to drone* button's dispatch gate + the green/red **Online/Offline** dot on that button. Since #217 the mission page no longer renders the two-pill `FieldLinkStatusChip` or the connection dialog — both the chip and the full link-signal panel now live only on the **Field Ops** page (`FieldHubPanel`, which still shows the per-signal RC / Telemetry / Hub / Broker pills). On the mission page the link state collapses to that single dot: green when `hub_online`, red otherwise.

#### Send to drone (mission dispatch)

`POST /api/v1/missions/{id}/dispatch` pushes the mission KMZ into the field hub's wayline library, which DJI Pilot 2 syncs into its route list over the local network (`docs/specs/FIELD-HUB.md` §4.2). The route reuses the export pipeline (`export_mission(db, id, ["KMZ"], ...)`), so dispatch inherits the VALIDATED/EXPORTED/MEASURED export gate, the VALIDATED → EXPORTED transition, and the 409 altitude-clamp gate. Since #217 the request body mirrors the export endpoint's **full** option set — `include_geozones`, `include_runway_buffers`, `dji_heading_mode_override`, and `acknowledge_altitude_clamps` — and forwards them into `export_mission`, so the dispatched KMZ is byte-for-byte what a download of the same config would produce (export ⇄ dispatch share one engine, diverging only at delivery). Before #217 the dispatched KMZ silently dropped geozones and ignored an unsaved heading-mode pick. The `DISPATCH` audit row records the geozone flags + heading override alongside `wayline_id`. A measured mission can re-dispatch and stays MEASURED (the transition keys on VALIDATED). The service posts the KMZ + metadata to the hub's shared-secret internal endpoint, then upserts a `wayline_dispatch` row keyed on the unique `mission_id`, so a re-dispatch updates the existing record with a stable wayline uuid and Pilot sees an updated route instead of a duplicate. `drone_model_key` / `payload_model_keys` derive from `DJI_WPML_ENUMS` (M4T fallback for unmapped drones) so Pilot's connected-aircraft filter matches the enums baked into the KMZ; `sign` is the KMZ md5. An unreachable or unconfigured hub raises 502 and nothing persists — the status transition and the dispatch row roll back with the uncommitted session. Each successful dispatch commits a `DISPATCH` audit row in the same transaction.

Frontend: since #217 there is no separate *Send to drone* card — a **Send to Drone** button renders directly under **Download Export** inside the export section (`ExportFormatSection`), with a centered label and the green/red Online/Offline dot. `ExportPanel` builds both the download and the dispatch payload from one shared `buildExportOptions()`, so the two always match. The button is gated only on the hub being reachable (`hub_online`) + status ∈ {VALIDATED, EXPORTED, MEASURED} — **not** on a drone being online, so waylines can be staged before the aircraft is up (Pilot pulls the route library over HTTP whenever it next connects). A tooltip explains the blocker, it shows inline success/error feedback (the backend `detail` message is surfaced verbatim), and a clamp 409 turns the button into an explicit "Acknowledge clamps and send" retry. The page refetches after a successful dispatch because status may have moved to EXPORTED (a measured mission stays MEASURED). Strings under `mission.sendToDrone.*` (EN + SK). Field-hub *configuration* (connect address, QR, CA cert) is no longer on the mission page — it moved to the Field Ops page (see below).

#### Field hub connection panel (Field Ops page)

`FieldHubPanel` (`frontend/src/components/mission/FieldHubPanel.tsx`) helps the operator point DJI Pilot 2 at the hub without reading the address off a terminal. Since #217 it is no longer reachable from the mission page (the `FieldHubDialog` modal wrapper that opened it there was removed); it now renders only as the left card of the **Field Ops** page. It consumes that page's single `useFieldLinkStatus` poll result — no second poll — and shows, in one place: all four link **signals** (Hub / RC / Broker / Telemetry; since #109) plus a **Heartbeat Check** button that force-refreshes the status on demand and a "last checked" timestamp; the device-facing **connect address** (`http://<host>:8080` since #109, from `status.connect_url`) with a copy button; an **inline-rendered QR** of that address for scanning on the RC, generated by a vendored dependency-free encoder (`frontend/src/utils/qrcode.ts`, exporting `encodeQrMatrix` / `qrMatrixToPath`), so `package-lock.json` stays untouched; the **connected-device list** (model + serial + online state, with an empty state); and a **CA-certificate download** (`GET /api/v1/field-link/ca-cert` through the JWT client, since a plain `<a href>` can't carry the bearer) — vestigial under the HTTP posture, the endpoint 404s when no CA path is set. Graceful states: hub offline → an offline troubleshooting hint; online but no host configured → "hub address not configured"; before the first poll (`status === null`) → a connecting state. EN + SK strings under `mission.fieldHub.*`. See `docs/specs/FIELD-HUB.md` (top status + §6).
...
Frontend: the *Upload Drone Media* header button on the Validation & Export page (the MissionTabNav action slot) opens `UploadDroneMediaDialog`, scoped to the current mission. The dialog lists the mission's inspections (labelled `Inspection {order} · {method}`) plus an *Unassigned* bucket, each with a file-count badge. Each inspection group has a drop-or-browse zone that uploads video files (presigned PUT → complete-upload); files can be reordered within an inspection and dragged between inspections (`@dnd-kit`), and manual rows carry a trash button that deletes them. Inspection assignment + order survive a reload. Empty states: "This mission has no inspections yet. Add an inspection before uploading media." when the mission has none, "No files yet" per empty group. Load/upload/move/reorder/delete failures surface as an inline error line. A single footer **Confirm** button fires one measurement per inspection-with-media at once (`Promise.allSettled` over `createMeasurement` → `POST /api/v1/inspections/{inspection_id}/measurement`) — skipping empty groups and the unassigned bucket, settling every call so one failure can't abort the rest. It registers the started run ids with the measurement-progress context (the corner progress toast), navigates to the mission's Results tab (`/operator-center/missions/{mission_id}/results`), and closes; if every run fails to start it shows an inline error and stays open. There is no per-inspection measure button anymore. Strings under `mission.uploadDroneMediaDialog.*` plus the button label `mission.validationExportPage.uploadDroneMedia` (EN + SK); types in `frontend/src/types/droneMedia.ts` mirror `backend/app/schemas/drone_media.py` + the media shapes in `schemas/field_link.py`.

#### Field Ops page (cloud missions + drone media)

`FieldOpsPage` (`/operator-center/field-ops`, nav item `nav.fieldOps`, added in #162) is the operator surface for the field hub's cloud-route library and the media returned from flights (`docs/specs/FIELD-HUB.md` top status + §5/§6). Since #179 it uses the standard 30/70 shell: a left **Field Hub connection** card (30%) over a right column (70%) carrying the two tables, each card with its own **Refresh**. The left card renders the shared `FieldHubPanel` off a single `useFieldLinkStatus()` poll — the same component the connection dialog uses (see "Field hub connection dialog" above) — so the hub's live signals, heartbeat, connect address/QR, device list, and CA-cert download sit beside the tables and the offline state lives in that card (`field-hub-offline`) rather than a banner. Two tables:

- **Cloud Missions** — the waylines registered on the hub, with a per-row delete that removes the row *and* its KMZ object. Backed by `GET` / `DELETE /api/v1/field-link/waylines`, an operator proxy over the hub's shared-secret `GET` / `DELETE /internal/api/v1/waylines`. The list degrades to empty when the hub is unconfigured/unreachable (offline state shows in the left Field Hub card); delete is 404 when the hub reports the wayline absent, 502 when the hub is down.
- **Drone Media** — the transferred files (filename, status, size, matched mission, captured time). Each row has a presigned view/download url (`GET /api/v1/drone-media/{media_id}/view-url`) opened in a new tab, plus a *Link & process* control: a mission + inspection picker (inspection auto-selected when the mission has exactly one) that runs `assignDroneMedia` → `moveDroneMedia` → `createMeasurement` in order, so the video is linked to the inspection and its measurement starts in one step, then offers a link to the results page. A measurement consumes **all** videos currently assigned to the chosen inspection, in order.

EN + SK strings under `fieldOps.*`; the wayline types in `frontend/src/types/fieldLink.ts` (`FieldLinkWayline`, `FieldLinkWaylineListResponse`) mirror `backend/app/schemas/field_link.py`. Folded-in field-test fixes: the hub pins the MinIO presign region so wayline KMZ download signs offline, and the media-events ingest scrubs nul/control bytes from `captured_at` + the raw callback and drops non-finite / out-of-range shoot positions so a dirty device callback degrades instead of 422ing.
...
- **Measurement** (`backend/app/models/measurement.py`, the ORM model) — aggregate for one measurement run. Owns the status machine via `transition_to()` / `fail(msg)`, snapshots reference points off the inspection's LHAs at create (plus the configured glide slope + its tolerance off the parent AGL), confirms operator light boxes (`confirm_boxes()`), rolls measured transition angles up to PASS/FAIL (`score_light()` / `with_summaries_from()`), and bands the measured glidepath against the snapshotted glide slope ± tolerance (`glide_slope_within_tolerance(measured_angle)`, None when the angle, tolerance, or measurement is missing). Persisted directly as the ORM row; the heavy per-frame results blob lives in object storage, referenced only by `object_key`.
...
- `Mission.POST_PLAN_STATUSES` — `frozenset({VALIDATED, EXPORTED})`; the auto-regress branch in `generate_trajectory`, plus the `mark_measured()` / `regress_to_planned()` gates. No longer the export gate - that moved to `EXPORT_ELIGIBLE_STATUSES`.
- `Mission.EXPORT_ELIGIBLE_STATUSES` — `frozenset({VALIDATED, EXPORTED, MEASURED})`; the export/dispatch gate (`export_mission` rejects anything else, dispatch reuses it). MEASURED is post-validation - its plan + artifacts persist, so re-download / re-send stays legal. Deliberately a separate frozenset from `POST_PLAN_STATUSES` so widening export eligibility never touches `mark_measured` / regress / auto-regress, and the `VALIDATED → EXPORTED` bump still keys on `VALIDATED` so a measured mission stays MEASURED after export.
...
Backend enums live in `backend/app/core/enums.py`. Frontend literal unions in `frontend/src/types/enums.ts` must stay in lockstep — `backend/tests/test_enum_parity.py` enforces parity for `ConstraintType`, `MissionStatus`, `WaypointType`, and `PapiCenterHeightReference` to guard against drift. When adding/renaming an enum member, update both files in the same change.
