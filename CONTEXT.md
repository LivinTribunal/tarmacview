...
- **Field profile** — the docker compose profile (`docker compose --profile field up`) that adds `fieldhub`, EMQX (MQTTS broker), and MinIO (S3-compatible object store) to the stack. The default three-container stack is unaffected when the profile is off.
