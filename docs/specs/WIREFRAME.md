...
- **Export section:**
  - **Field-link status chip** — pill at the top of the export panel showing the RC link state from the field hub. Three states: grey "Field hub not connected" (no hub configured or reachable), red "RC offline" (hub up, no online devices), green "RC connected – {model}" (aircraft model preferred over the RC gateway in the label; plain "RC connected" when the model is unknown). `ExportPanel` polls `GET /api/v1/field-link/status` every 10 s while mounted (`useFieldLinkStatus`) and shares the one result between the chip and the Send to Drone card's dispatch gate; the chip is presentational (`status` prop) and stays hidden until the first response. A failed poll degrades to the no-hub state. EN + SK strings under `mission.fieldLink.*`. See SPEC.md "Field-link status chip" and `docs/specs/FIELD-HUB.md` §6.
...
**Layout:** `--tv-*` card stack with a header showing the run name (operator `label` when set, else the inspection fallback `Inspection N · Method` computed from the `inspection_method` + `inspection_sequence_order` the results payload now carries), the run status, and three buttons: **Download PDF** (→ `GET /measurements/{id}/pdf-report`), **Rename** (modal → `PATCH /api/v1/measurements/{id}`; blank clears to the fallback), and **Delete** (confirm modal → `DELETE /api/v1/measurements/{id}`, then navigates back to the list, Page 15). Content renders only once `has_results` is true; otherwise a "not ready yet" card. Four content blocks:
