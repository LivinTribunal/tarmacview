...
`FieldOpsPage` (`/operator-center/field-ops`, nav item `nav.fieldOps`, added in #162) is the operator surface for the field hub's cloud-route library and the media returned from flights (`docs/specs/FIELD-HUB.md` top status + §5/§6). Two tables:

- **Cloud Missions** — the waylines registered on the hub, with a per-row delete that removes the row *and* its KMZ object. Backed by `GET` / `DELETE /api/v1/field-link/waylines`, an operator proxy over the hub's shared-secret `GET` / `DELETE /internal/api/v1/waylines`. The list degrades to empty (and shows an offline banner) when the hub is unconfigured/unreachable; delete is 404 when the hub reports the wayline absent, 502 when the hub is down.
