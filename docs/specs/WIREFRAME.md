...
1. **PAPI Vertical Analysis** (`papi-vertical`) — the three per-light charts in a 2-col grid: `LightAngleChart` (per-light angle over the climb, shading the white/red transition zones via recharts `ReferenceArea` from `transition_angle_min/middle/max`), `IntensityChart`, `ChromaticityChart`.
2. **PAPI Horizontal Analysis** (`papi-horizontal`) — a placeholder anchor with an empty card; the chart work is deferred to a follow-up.
...
**States:** loading spinner, error card, and the `has_results: false` "not ready" card. Strings under `results.*` (incl. `results.sections.*` / `results.summary.*` / `results.verdictRollup.*` / `results.perPapi.*` / `results.glidePath.*`) + `measurement.*` (EN + SK).
