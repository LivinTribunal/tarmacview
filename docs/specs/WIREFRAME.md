...
**Surveyed MEHT height (AGL fields):**
- A PAPI-only `meht_height_m` number input rides on the AGL feature editor (`featureInfo/AglFields`) — the surveyed/published minimum eye height over the threshold in meters. When set, MEHT_CHECK and APPROACH_DESCENT fly this height directly; when left blank the trajectory derives it from `distance_from_threshold · tan(glide_slope)`, so an unset field is behaviour-neutral. Hidden for edge-lights AGLs and nulled on a PAPI → edge-lights switch. Coordinator-only ground truth (writes go through the coordinator-gated `.../agls` routes). Labels: `coordinator.detail.aglMehtHeight` / `aglMehtHeightHelp` (EN + SK).
- The MEHT_CHECK inspection-config readout (`computed-meht-height`) is surveyed-aware: it shows the surveyed `meht_height_m` of the first selected PAPI AGL when set, else the derived value — resolved through `resolveMehtHeight` (`@/utils/mehtHeight`).
...
**Source:** no new endpoints. `listAirportMeasurements(airportId)` filtered client-side to the current mission maps each inspection → its **latest** run (the API returns newest-first, so the first row seen per `inspection_id` wins). `listInspectionTemplates({ airport_id })` backs the picker row names. `getMeasurementResults(id)` loads the selected inspection's pivoted per-frame payload. `downloadMeasurementReport(id)` backs the header PDF button.

**Header (MissionTabNav action slot):** a **Save** pill + Saved-timestamp copied for visual parity with the other tabs but permanently disabled (nothing is saved here), and a real **Download PDF Report** button (`secondary` compute-context with a `FileText` icon) targeting the selected run, or — when nothing is selected but other inspections are measured — the first DONE run; disabled when no inspection is measured.

**Left panel** (`ResultsLeftPanel`) — a stack of `--tv-*` cards:
1. **Inspection picker** (`InspectionPicker`, adapted from the Configuration `InspectionList`) — a collapsible, single-select list of **all** the mission's inspections, sorted by `sequence_order`, each row carrying a sequence badge, the template name, a method badge, and a **result tag**. A **DONE** run shows the `MeasurementStatusChip` plus a `pass/total` rollup and is **selectable** (clicking selects it, clicking the selected row deselects). An **AWAITING_CONFIRM** run — an uncertain PAPI detection parked for manual box review — shows the status chip plus a warning-toned **"Review"** affordance (`results.picker.review`) and is **reviewable**: clicking it opens the `MeasurementFlowDialog` via the picker's `onReview` prop instead of selecting. Every other in-progress run (QUEUED / FIRST_FRAME / PROCESSING) shows its status chip and an un-measured inspection shows a muted "Not measured" tag; both render **disabled** (`aria-disabled`, not clickable).
...
**Manual box review (AWAITING_CONFIRM):** clicking a reviewable picker row opens `MeasurementFlowDialog` — the same first-frame box-confirm step that used to live on the deleted measurements list page. `MissionResultsPage` owns the dialog's open state (the reviewed inspection/run) and mounts the dialog; confirming the boxes calls `confirmMeasurementLights`, advancing the run `AWAITING_CONFIRM → PROCESSING` server-side. On close the page refetches its mission-scoped run list so the row flips from the "Review" affordance to the `PROCESSING` chip; cancelling closes with no change. No new endpoints — the dialog reuses `GET /measurements/{id}` (status/preview) and `POST /measurements/{id}/confirm-lights`.

**Default selection & empty states:** on load the **first measured (DONE)** inspection (by sequence) is auto-selected, unless an `?inspection=` query points at a DONE inspection. Right-column states: no inspection measured at all → "no data" message (`results-no-data`, Download PDF disabled); a measured inspection exists but none is selected (operator deselected) → "pick an inspection" message (`results-pick-inspection`, Download PDF stays enabled); selected run still loading → spinner; load failure → error; `has_results: false` → "not ready" card; otherwise the results body (`ResultsPage`).
