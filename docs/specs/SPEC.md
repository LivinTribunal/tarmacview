...
Frontend: `SendToDroneSection` card in the export panel. Since #109 the button is gated only on the hub being reachable (`hub_online`) + status ∈ {VALIDATED, EXPORTED} — **not** on a drone being online, so waylines can be staged before the aircraft is up (Pilot pulls the route library over HTTP whenever it next connects). A tooltip explains the blocker, it shows inline success/error feedback (the backend `detail` message is surfaced verbatim), and a clamp 409 turns the button into an explicit "Acknowledge clamps and send" retry. The page refetches after a successful dispatch because status may have moved to EXPORTED. Strings under `mission.sendToDrone.*` (EN + SK). The card header also carries a *Field Hub* button that opens the connection dialog below.
