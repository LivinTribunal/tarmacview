...
class InfoOverlayRenderer:
    ...
    @staticmethod
    def add_angle_overlay(frame: np.ndarray, frame_number: int,
        ...
        Args:
            frame_measurements: Pre-computed measurements for current frame (OPTIMIZATION - avoids recalculating angles)
        """
        height, width = frame.shape[:2]
        ...
        cv2.rectangle(frame, (0, panel_y_start), (width, panel_y_end),
                     (255, 255, 255), -1)
            ...
            try:
                ...
                if frame_number == 0:
                    ...
                    if frame_measurements:
                        logger.info(f"  frame_measurements keys: {list(frame_measurements.keys())[:20]}...")  # First 20 keys
                ...
                for papi_name in ['PAPI_A', 'PAPI_B', 'PAPI_C', 'PAPI_D', 'PAPI_E', 'PAPI_F', 'PAPI_G', 'PAPI_H']:
                    ...
                    if angle is None and papi_name in reference_points:
                        ...
                        ground_dist = haversine_distance(
                            float(drone_data['latitude']), float(drone_data['longitude']),
                            float(papi_point.get('latitude', 0)), float(papi_point.get('longitude', 0))
                        )
                        # Use elevation_wgs84 with fallback to elevation/altitude for backward compatibility
                        drone_elevation_wgs84 = float(drone_data.get('elevation_wgs84', drone_data.get('altitude', drone_data.get('elevation', 0))))
                        papi_elevation_wgs84 = float(papi_point.get('elevation_wgs84', papi_point.get('elevation', 0)))
                ...
                if 'TOUCH_POINT' in reference_points:
                    ...
                    ground_dist = haversine_distance(
                        float(drone_data['latitude']), float(drone_data['longitude']),
                        float(touch_point.get('latitude', 0)), float(touch_point.get('longitude', 0))
                    )
                    # Use elevation_wgs84 with fallback to elevation/altitude for backward compatibility
                    drone_elevation_wgs84 = float(drone_data.get('elevation_wgs84', drone_data.get('altitude', drone_data.get('elevation', 0))))
                    touch_elevation_wgs84 = float(touch_point.get('elevation_wgs84', touch_point.get('elevation', 0)))
                ...
                cv2.putText(frame, "GPS POSITION", (x_col1, y_base),
                           font, 1.0, (80, 80, 80), 2)
                # Convert to float to handle Decimal types from DynamoDB
                cv2.putText(frame, f"Lat: {float(drone_data['latitude']):.6f}", (x_col1, y_base + 45),
                           font, 0.8, (60, 60, 60), 2)
                cv2.putText(frame, f"Lon: {float(drone_data['longitude']):.6f}", (x_col1, y_base + 85),
                           font, 0.8, (60, 60, 60), 2)
                # Use elevation_wgs84 with fallback for backward compatibility
                elevation_display = float(drone_data.get('elevation_wgs84', drone_data.get('altitude', drone_data.get('elevation', 0))))
                cv2.putText(frame, f"Alt: {elevation_display:.1f}m WGS84", (x_col1, y_base + 125),
                           font, 0.8, (60, 60, 60), 2)
                ...
                cv2.putText(frame, f"Time: {minutes:02d}:{seconds:05.2f}", (x_col1, y_base + 165),
                           font, 0.8, (60, 60, 60), 2)
                cv2.putText(frame, f"Frame: {frame_number + 1}", (x_col1, y_base + 205),
                           font, 0.8, (60, 60, 60), 2)

                # Column 2: PAPI Vertical Angles with all angle details
                x_col2 = col_width + 25
                cv2.putText(frame, "PAPI ANGLES", (x_col2, y_base),
                           font, 1.0, (80, 80, 80), 2)
                ...
                for papi_name, angle in papi_angles.items():
                    ...
                    transition_mid = papi_ref.get('transition_angle_middle')
                    ...
                    if transition_start is not None and transition_end is not None:
                        ...
                        cv2.putText(frame, trans_text, (x_col2, y_papi),
                ...
                if touch_angle is not None:
                    ...
                    cv2.putText(frame, "TOUCH POINT", (x_col3, y_base),
                               font, 1.0, (80, 80, 80), 2)
                    cv2.putText(frame, f"Current Angle: {touch_angle_float:.2f}", (x_col3, y_base + 45),
                               font, 0.8, (0, 180, 180), 2)

                    # Get glide path angle from touch point reference - convert to float
                    touch_ref = reference_points.get('TOUCH_POINT', {})
                    glide_path_angle = float(touch_ref.get('nominal_angle', touch_ref.get('glide_path_angle', 3.0)) or 3.0)
                    tolerance = float(touch_ref.get('tolerance', 0.1) or 0.1)

                    cv2.putText(frame, f"Nominal GP: {glide_path_angle:.2f} (+/-{tolerance:.2f})", (x_col3, y_base + 85),
                               font, 0.8, (0, 150, 0), 2)

                    # Calculate deviation from glide path
                    deviation = touch_angle_float - glide_path_angle
                    dev_color = (0, 255, 0) if abs(deviation) <= tolerance else (0, 165, 255) if abs(deviation) <= tolerance * 2 else (0, 0, 255)
                    cv2.putText(frame, f"Deviation: {deviation:+.2f}", (x_col3, y_base + 125),
                               font, 0.8, dev_color, 2)
                    ...
                    cv2.putText(frame, status_text, (x_col3, y_base + 165),
                               font, 0.7, status_color, 2)
                else:
                    cv2.putText(frame, "TOUCH POINT", (x_col3, y_base),
                               font, 1.0, (80, 80, 80), 2)
                    cv2.putText(frame, "No data", (x_col3, y_base + 45),
                ...
                logger.error(f"reference_points keys: {list(reference_points.keys()) if reference_points else 'None'}")
                logger.error(f"frame_measurements: {frame_measurements}")
                # Render error message on the frame instead of crashing
                cv2.putText(frame, f"ERROR: {error_msg[:80]}", (25, panel_y_start + 50),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        else:
            # Display placeholder when no reference points
            cv2.putText(frame, "Angles: No ref points", (25, panel_y_start + 50),
