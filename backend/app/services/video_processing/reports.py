...
class PAPIReportGenerator:
    ...
    def generate_html_report(
        ...
        enhanced_main_video_path: str = None,
    ) -> str:
        """Generate interactive HTML report with Plotly charts,
        embedded videos, and reference point coordinates"""
        try:
            ...
            # Save report
            report_filename = (
                f"papi_report_{session_data.get('session_id', 'unknown')}_"
    ...
    def _create_html_template(
        ...
        """Create HTML template with Plotly charts,
        embedded videos, and reference point coordinates"""
        ...
        for papi_id in ["PAPI_A", "PAPI_B", "PAPI_C", "PAPI_D"]:
            if papi_id in papi_data and papi_data[papi_id]["r_values"]:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=timestamps, y=data["r_values"], name="Red", line=dict(color="red")
                    ),
                    row=1,
                    col=1,
                )
                fig.add_trace(
                    go.Scatter(
                        x=timestamps, y=data["g_values"], name="Green", line=dict(color="green")
                    ),
                    row=1,
                    col=1,
                )
                fig.add_trace(
                    go.Scatter(
                        x=timestamps, y=data["b_values"], name="Blue", line=dict(color="blue")
                    ),
                    row=1,
                    col=1,
                )
                ...
                fig.add_trace(
                    go.Scatter(
                        x=timestamps,
                        y=data["ground_distance"],
                        name="Ground Distance (m)",
                ...
                fig.update_layout(title=f"{papi_id} Analysis", height=600, showlegend=True)
                ...
                if papi_id in video_paths and video_paths[papi_id]:
                    ...
                    video_src = (
                        f"{settings.API_BASE_URL}/api/v1/papi-measurements/"
                        f"session/{session_data.get('session_id')}/papi-video/{light_name}"
                    )
                    video_html = f'''
        ...
        enhanced_video_url = self._get_enhanced_video_url(
            session_data.get("session_id"), enhanced_main_video_path
        )
        html_template = f"""
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <style>
                ...
                .reference-table th
                {{ background-color: #007bff; color: white; padding: 12px 8px; }}
                ...
                .original-video-container video
            ...
            <div class="original-video-section">
                ...
                <div class="original-video-container">
                    <video width="800" height="600" controls>
                        <source src="{enhanced_video_url}" type="video/mp4">
                        Your browser does not support the video tag.
                    </video>
                    <p class="video-description">
                        Enhanced video with real-time drone position
                        information (latitude, longitude, elevation),
                        PAPI light status rectangles, angles to each
                        PAPI light and touch point, and frame progress.
                        The rectangles change color based on light status:
                        Red for red lights, White for white lights,
                        Orange for transition, Gray for not visible.
                        Angle measurements show elevation angles from
    ...
    def _get_enhanced_video_url(self, session_id: str, enhanced_main_video_path: str) -> str:
        ...
        if enhanced_main_video_path and os.path.exists(enhanced_main_video_path):
            ...
            return (
                f"{settings.API_BASE_URL}/api/v1/papi-measurements/"
                f"session/{session_id}/enhanced-video"
            )
        else:
            # Fallback to original video
            return (
                f"{settings.API_BASE_URL}/api/v1/papi-measurements/"
                f"session/{session_id}/original-video"
            )
    ...
    def _create_summary_table(self, papi_data: Dict, timestamps: List[float]) -> str:
        ...
        for papi_id in ["PAPI_A", "PAPI_B", "PAPI_C", "PAPI_D"]:
            if papi_id in papi_data and papi_data[papi_id]["status"]:
                ...
                avg_intensity = (
                    sum(data["intensity"]) / len(data["intensity"]) if data["intensity"] else 0
                )
                avg_angle = sum(data["angles"]) / len(data["angles"]) if data["angles"] else 0
                avg_distance = (
                    sum(data["ground_distance"]) / len(data["ground_distance"])
                    if data["ground_distance"]
                    else 0
                )
    ...
    def _create_reference_points_table(self, reference_points: Dict) -> str:
        ...
        if not reference_points:
            return (
                '<p style="color: #666; font-style: italic;">No reference point data available.</p>'
            )
