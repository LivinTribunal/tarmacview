...
class TwoPassProcessor:
    ...
    def __init__(self, output_dir: str, batch_size: int = None, progress_callback=None):
        ...
        # Initialize measurement collector for two-pass processing (pass progress callback for cancellation support)
    ...
    @staticmethod
    def create_combined_papi_video(papi_video_paths: Dict[str, str], output_path: str, fps: int = 30):
        """
        ...
        while True:
            ...
            # PAPI_A: leftmost
            combined_frame[0:settings.VIDEO_GEN_PAPI_HEIGHT,
                          0:settings.VIDEO_GEN_PAPI_WIDTH] = frames['PAPI_A']

            # PAPI_B: second from left
            combined_frame[0:settings.VIDEO_GEN_PAPI_HEIGHT,
                          settings.VIDEO_GEN_PAPI_WIDTH:settings.VIDEO_GEN_PAPI_WIDTH*2] = frames['PAPI_B']

            # PAPI_C: second from right
            combined_frame[0:settings.VIDEO_GEN_PAPI_HEIGHT,
                          settings.VIDEO_GEN_PAPI_WIDTH*2:settings.VIDEO_GEN_PAPI_WIDTH*3] = frames['PAPI_C']

            # PAPI_D: rightmost
            combined_frame[0:settings.VIDEO_GEN_PAPI_HEIGHT,
                          settings.VIDEO_GEN_PAPI_WIDTH*3:combined_width] = frames['PAPI_D']
    ...
    def generate_videos_from_measurements(self, video_path: str, session_id: str,
                                         ...
                                         runway_heading: float, fps: int = 30,
                                         gps_cache: Dict = None,
                                         tracked_positions_cache: Dict = None) -> tuple:
        """
        ...
        if gps_cache is None:
            ...
            logger.info(f"Using cached GPS data from Pass 1: {len(gps_cache)} frames (OPTIMIZATION)")
        ...
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        enhanced_writer = cv2.VideoWriter(enhanced_path, fourcc, video_fps, (frame_width, extended_height))

        if not enhanced_writer.isOpened():
            raise ValueError(f"Failed to create enhanced video writer: {enhanced_path}")

        logger.info(f"Enhanced video: {frame_width}x{extended_height} (original: {frame_width}x{frame_height}, panel: {panel_height}px)")
        ...
        for light_name in ['PAPI_A', 'PAPI_B', 'PAPI_C', 'PAPI_D']:
            ...
            papi_writer = cv2.VideoWriter(papi_path, fourcc, video_fps, (settings.VIDEO_GEN_PAPI_WIDTH, settings.VIDEO_GEN_PAPI_HEIGHT))
        ...
        while True:
                ...
                for light_name in ['PAPI_A', 'PAPI_B', 'PAPI_C', 'PAPI_D']:
                    ...
                    if f'{light_key}_angle' in measurement_data:
                        frame_measurements[light_name] = {
                            ...
                            'horizontal_angle': measurement_data.get(f'{light_key}_horizontal_angle'),
                        ...
                        frame_measurements[f'{light_key}_transition_angle_min'] = measurement_data.get(f'{light_key}_transition_angle_min')
                        frame_measurements[f'{light_key}_transition_angle_middle'] = measurement_data.get(f'{light_key}_transition_angle_middle')
                        frame_measurements[f'{light_key}_transition_angle_max'] = measurement_data.get(f'{light_key}_transition_angle_max')
            ...
            for light_name in ['PAPI_A', 'PAPI_B', 'PAPI_C', 'PAPI_D']:
                ...
                if frame_number < len(measurements_data):
                    ...
                    logger.warning(f"No cached dimensions for {light_name} frame {frame_number}, using tracker")
                ...
                if light_frame.size > 0:
                    ...
                    scale_x = float(settings.VIDEO_GEN_PAPI_WIDTH) / original_region_width if original_region_width > 0 else 1.0
                    scale_y = float(settings.VIDEO_GEN_PAPI_DISPLAY_HEIGHT) / original_region_height if original_region_height > 0 else 1.0
                    ...
                    if red_mask is not None and red_mask.size > 0:
                        ...
                        cv2.drawContours(light_frame_resized, contours, -1, (settings.VIDEO_GEN_CONTOUR_COLOR_R, settings.VIDEO_GEN_CONTOUR_COLOR_G, settings.VIDEO_GEN_CONTOUR_COLOR_B), settings.VIDEO_GEN_CONTOUR_THICKNESS)
                    ...
                    final_frame[settings.VIDEO_GEN_PAPI_DISPLAY_HEIGHT:settings.VIDEO_GEN_PAPI_HEIGHT, 0:settings.VIDEO_GEN_PAPI_WIDTH] = [settings.VIDEO_GEN_FOOTER_COLOR_B, settings.VIDEO_GEN_FOOTER_COLOR_G, settings.VIDEO_GEN_FOOTER_COLOR_R]
                    ...
                    cv2.putText(final_frame, header_text, (10, 310),
                               font, 0.38, (50, 50, 50), 1)
                    ...
                    cv2.putText(final_frame, txt, (10, info_y),
                              font, 0.4, (0, 0, 200), 1)
            ...
            if frame_number % 30 == 0:
                ...
                if self.progress_callback:
                    self.progress_callback(50 + progress * 0.5, f"generating_videos_frame_{frame_number}")
        ...
        all_papi_lights_path = self.create_combined_papi_video(papi_paths, combined_papi_path, video_fps)
    ...
    def process_video_two_pass(self, video_path: str, session_id: str,
        ...
        measurements_data, gps_cache, tracked_positions_cache = self.measurement_collector.collect_measurements_only(
