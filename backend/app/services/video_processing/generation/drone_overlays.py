...
class DroneOverlayRenderer:
    ...
    @staticmethod
    def add_drone_position_overlay(frame: np.ndarray, frame_number: int, 
        ...
        if real_gps_data:
            ...
            if interpolated_gps:
                ...
                logger.debug(f"Frame {frame_number}: Using real GPS data - Lat: {interpolated_gps.latitude:.6f}, Lon: {interpolated_gps.longitude:.6f}, Elevation (WGS84): {interpolated_gps.elevation_wgs84:.1f}m")
    ...
    def calculate_angles_to_targets(drone_data: Dict, reference_points: Dict = None) -> Dict:
        ...
        if drone_lat is None or drone_lon is None or drone_elevation_wgs84 is None:
            raise ValueError(f"Incomplete GPS data. Missing: " +
        ...
        for target_id, target_pos in reference_points.items():
            ...
            target_elevation_wgs84 = target_pos.get('elevation_wgs84', target_pos.get('elevation', 0))
            ...
            if target_lat is not None and target_lon is not None:
                ...
                logger.debug(
                    f"{target_id}: drone_elevation_wgs84={drone_elevation_wgs84:.1f}m, target_elevation_wgs84={target_elevation_wgs84:.1f}m, "
