...
- **Field profile** — the docker compose profile (`docker compose -f docker-compose.yml -f docker-compose.field.yml --profile field up`) that adds `fieldhub`, EMQX (MQTTS broker), and MinIO (S3-compatible object store) to the stack and wires the backend->hub link. The `-f docker-compose.field.yml` override is required - plain `docker compose --profile field up` brings up the services but leaves `FIELDHUB_URL`/`FIELDHUB_CA` empty, so the backend stays no-hub. The default three-container stack is unaffected when the profile is off.
