...
interface ExportFormatSectionProps {
  ...
  // send-to-drone button shares the export config; dispatch state lives in
  // the parent so the one useFieldLinkStatus poll drives the status dot.
  onSendToDrone: () => void;
...
export default function ExportFormatSection({
  ...
  const hubLive = linkStatusKnown && hubOnline;
  return (
        ...
        <div className="mt-3 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            <label className="flex items-center gap-1 text-xs font-medium text-tv-text-secondary">
              <span>{t("mission.validationExportPage.formatLabel")}</span>
              <InfoHint
                text={`${t(`mission.validationExportPage.${selectedFormat.descKey}`)}. ${t(
                  `mission.validationExportPage.capabilityNote.${selectedFormat.capabilityKey}`,
                )}`}
                label={t("mission.validationExportPage.formatLabel")}
                testId="hint-format"
              />
            </label>
            <select
          ...
          {!terminal && (
            <div className="flex flex-col gap-2">
              <button
                ...
                className="flex items-center justify-between w-full text-xs font-semibold text-tv-text-secondary"
                data-testid="advanced-options-toggle"
                aria-expanded={advancedOpen}
              >
                <span>{t("mission.validationExportPage.advancedOptions")}</span>
                {advancedOpen ? (
                  <ChevronUp className="h-4 w-4" />
                ) : (
                  <ChevronDown className="h-4 w-4" />
          ...
          {/* send to drone - reuses the same selected config; status dot
              driven by the parent's useFieldLinkStatus poll */}
          <Button
            variant="secondary"
            ...
            >
              <span
                ...
                className={`h-2 w-2 rounded-full ${
                  hubLive ? "bg-[var(--tv-success)]" : "bg-[var(--tv-error)]"
                }`}
              />
              {hubLive
                ? t("mission.sendToDrone.online")
                : t("mission.sendToDrone.offline")}
            </span>
          </Button>
          ...
          {dispatchFeedback?.kind === "error" && (
            ...
            >
              {dispatchFeedback.message ?? t("mission.sendToDrone.error")}
            </p>
          )}
          {dispatchFeedback?.kind === "clamps" && (
            <p
