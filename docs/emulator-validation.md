...
service**, not DJI's reference demo. The run-kit lives in `emulator/`. It is
throwaway test tooling — **cert-free, plain HTTP, no postgres** — separate from
the production `field` compose profile, and it is the supported way to validate
the connect/login flow without a real RC.
...
| V2 — wayline list render + filter + KMZ download | ✅ | HTTP wayline library + Pilot's route library |
...
loopback); the host LAN IP hairpins through Docker Desktop's proxy and resets.
Everything device-facing is funnelled through the single nginx port `8080`,
including the object store — so presigned SigV4 URLs are signed for
`10.0.2.2:8080` and nginx forwards that `Host` verbatim to MinIO (a rewritten
...
- At least one **TarmacView KMZ export** per fleet model you want to test. For a
  plumbing-only dry run any `.kmz` works.

## Step A — rebuild the stack from scratch

```bash
cp emulator/.env.emulator.example emulator/.env.emulator   # then fill in DJI app creds
# the field profile shares minio's console port - stop it first
docker compose --profile field stop
# wipe any previous run, rebuild fieldhub, start cert-free. --env-file is
# required: compose interpolates ${VAR} from it (environment: overrides
# env_file:, so --env-file is the one predictable path).
...
```bash
docker compose --env-file emulator/.env.emulator -f emulator/docker-compose.emulator.yml logs -f nginx
```
...
`emulator/.env.emulator`, then `up -d` again), then:
...
emulator/seed-wayline.sh /path/to/any.kmz "Smoke RWY22" 0-89-0 1-53-0
...
# 4. download via the presigned redirect - 307 -> object 200 (the SigV4 test)
WID=<id from step 3>
...
```bash
emulator/seed-wayline.sh exports/m300.kmz "M300 inspection"  0-60-0 1-53-0
emulator/seed-wayline.sh exports/m350.kmz "M350 inspection"  0-89-0 1-53-0
emulator/seed-wayline.sh exports/m3e.kmz  "M3E inspection"   0-77-0
emulator/seed-wayline.sh exports/m4t.kmz  "M4T inspection"   0-99-1   # the enum the demo rejected
```
...
Work down the connect page and the route library, watching the nginx log.
...
- **Pilot refuses `http://`** → run a TLS variant: `scripts/field-hub/gen-certs.sh 10.0.2.2`
...
- **nginx single-port is fussy** → BlueStacks can also reach individual host
  ...
  platform URL `http://10.0.2.2:8800`. The single nginx port is the proven
